...
**已实现（协议级，Python 3.11）**：**P1 最小 Core**（root 解析与卷身份守卫、SQLite registry 与迁移、只读 JSON 投影、`where` 确定性选择、`doctor` D1–D10、模拟事务与 journal 驱动恢复、CLI 与 golden 语料）；**管家域的全部步骤 1–9、11–12**（数据根注册与只读 PE 静态探测、能力白名单发现、`adopt --mode reference` 非拥有式登记、依赖分流与 `scope` 决策、会话激活与 **user 级环境变量持久化**（plan → approval → 写入 → 精确还原）、**steward-first `where`** 与数据根/reference 重观测诊断、**删除分级**（只删 AIROOT 自己装的，reference 永不 `uninstall`）、**能力边界**）；**Skill 适配层**（`SKILL.md` + `agents/airoot.json` + `references/`，带漂移守卫）；**可信来源清单与 Install Backend**；**`rebuild`**（派生状态可重建、权威不可自重建）；以及 **`search` 的协议面与 crawl 建的持久索引**（**它不是 USN 索引**：`freshness.current` 只表示"这份清单是最近一次遍历建立的"，与 journal 的"游标没断档"不是一回事）。**1457 项测试通过**（含 115 项常驻跨工件一致性检查 `cli/tests/test_l0_consistency.py`）。**P4 的真实归档安装已在真机走完一次**（§148：真实上游的 `cmake-3.31.6-windows-x86_64.zip` → 上游校验和 → 解压 **8146** 个条目 → 真跑 `cmake version 3.31.6` → `gc --apply` 真删；**`real_machine_acceptance.py --online` 的这个 phase 全链 PASS 也已观察到**（§148.7））。**P2 的第一阶段（线路面的客户端那一半）也已落地**（§108）：`caps/identity.py` 只读本进程 token 得出 `client` 块，`broker/protocol.py` 造/验 `broker-request`、解 `broker-response`（`broker_unavailable()` 报 `NOT_IMPLEMENTED`(1)，ADR-0025 的 D1）。**它不证明信任边界**：没有 broker、没有 named pipe、没有任何一处校验**别人**的 token，`client` 块仍然只是自述；这一阶段把两个 `broker-*` schema 从"没有任何使用者"变成"有使用者"，仅此而已。**§113 又打好了受保护边界需要、但不需要先有边界就能验证的三块地基**：Ed25519 校验（RFC 8032，纯 Python，不加依赖——**校验真了不等于本机能批准**，签发仍是那条边界）、读**别人** token 的 `probe_process`（观测，不是授权）、ACL 写一侧（`SetSecurityInfo`，**只作为库**，不接任何动词）。三者都不构成那个边界。

**逐阶段的实现记录**（`§31`–`§167`：每一阶段做了什么、发现什么缺陷、加了哪组守卫、怎么验红、哪些边界没有做）**全部在 `docs/AIROOT-v0.3-管家模型与数据根契约草案.md` 的对应小节里**；本文只保留当前状态、仓库地图与操作面。这是**有意**的：本文是**入口文档**，必须能被读者**完整**载入，而逐阶段细节曾让它长到超过读取预算并被**静默截断**（见 §54）——把细节留在这里，等于让读者拿到半份规则；转写到草案里则**不丢任何信息**（§1 里原来的 23 段在草案里各有一节，实测零独有，见 §54.2）。**本文最后一行是哨兵**：读到它=你拿到的是完整文档；**没读到它=你手里这份是被截断的副本**（§138：`≤65536` 这条守卫量的是文件，而预算是整个工作区共享的）。

**尚未实现**：ACL 的**强制**（写一侧已作为库落地，见下；缺的是调用者）、UAC、**受保护的** elevated broker（**服务器那一半**：P2 第一阶段的客户端线路面已落地，见 §108）、named-pipe（**§115 把线接上了，但接出来的不是那条边界**——读数、"它到底保护了什么"与"还缺哪三样"在草案 §115.4/§115.7）、machine PATH 写入（**稳定入口已交付，但 PATH 那一条不写**——ADR-0050）、**machine 级环境变量持久化**（`--scope machine` 现在报 `PRIVILEGE_REQUIRED`）、**签名/来源证明**（TUF / Sigstore 级；来源清单与摘要校验已就绪，v1 只做 digest）、**真实工具的安装**（**对真实上游已经走完一次**：§117 的本机签发 + 真机 `FINALIZED`、§118 的 rustup 真机执行。**但那个工具链不在 AIROOT 的账上**：它在 rustup 自己的默认位置，是 AIROOT 装的**安装器**写的，所以 `where rust-toolchain` 仍指向 store 里的 `rustup-init.exe`——要让 AIROOT 管它，走 `discover`/`adopt`，不是让 `where` 去猜）、**`file_search` 的 Native Index**（USN journal / NTFS 元数据 / 常驻索引器；协议面 §31 与 crawl 建的持久索引 §32 已可用）、Everything adapter、内容搜索、跨用户 ACL 过滤、`reconcile`、`path backup\|restore`、`root adopt\|relocate`（已按命令路径登记在 `agents/airoot.json`）、**受保护签发方**（签发本身已经有了：`airoot issue`，ADR-0046/ADR-0049；受保护的那一半是**私钥不可被用户自己的进程读取**，由 ADR-0044 裁决为在本机做不到，理由见 §8）、`.ai/tooling.json` 的写入。管家域 §3–§15 的**全部步骤 1–9、11–12 均已落地**；只剩依赖 P2 的 machine 级环境变量（步骤 10），以及 **ACL 的写一侧**——`doctor` 侧的**观测与漂移发射已落地**（§58 / ADR-0023：`caps/acl.py` 只读 DACL，无需提权），**写一侧也已作为库落地**（§113 / ADR-0040：baseline/apply/verify/restore，`SetSecurityInfo`，非空基线保护式写入、空基线作为发现被拒），仍缺的是**接上它的调用者**：把基线强加回目录要有 broker，以及一条"调用者写完还活着"的判据，今天两者都不存在（§114 只把这条判据要读的**别人 token**补齐了）。
...
| `docs/AIROOT-v0.3-实现决策记录.md` | **ADR 日志（ADR-0001 … ADR-0066）**：语言路线（Python P1 → Rust P2）、三处 Schema 缺陷处置、**ADR-0004 管家模型**、ADR-0021 放宽优先、ADR-0025 的 D1–D9。**括号里是范围，不是内容清单**——每一条 ADR 自己的标题才是它的权威，别在这里补一份手抄的目录（§85） |
...
| `docs/AIROOT-v0.3-管家模型与数据根契约草案.md` | **提案 + 逐阶段实现记录（§1–§30 是契约章节，§31–§167 是每一阶段的实现记录）**：reference-first 管家模型、数据根、能力白名单、`where` 优先级改写、**依赖分流与确认、环境变量持久化、删除语义分级、能力边界声明** |
...
| `cli/app/airoot/` | **P1 CLI Core**（Python 包）：`cli.py` 入口、`registry/`、`tx/`、`caps/`、`ext/`、`broker/`（P2 起）、`crypto/`（§113 起）。`tx/` 里 `rollback.py` 是**回滚语义的唯一实现**（两个 runner 都调它，§109 / ADR-0035），`registration.py` 是同一做法的第二个例子（注册 + 收走闩锁的纠正，§164 / ADR-0063） |
| `cli/app/airoot/caps/` | **能力域模块总览**（28 个，逐个职责见草案 §138.2）：`acl.py`、`boundary.py`、`desired.py`、`discovery.py`、`doctor.py`、`effective.py`、`environment.py`、`exposure.py`、`health.py`、`identity.py`、`inventory.py`、`launcher.py`、`layout.py`、`lifecycle.py`、`pathexposure.py`、`planner.py`、`probe_pe.py`、`rebuild.py`、`runtime.py`、`search.py`、`searchindex.py`、`selection.py`、`session.py`、`sources.py`、`toolstate.py`、`usn.py`、`version.py`、`where.py`（`backends/` 见下行）。读代码前必须知道的三条：`layout.py` 是 payload 标记扫描的**唯一定义**（`doctor`/`rebuild` 共读，§71）；`acl.py` 的写一侧与 `broker/policy.py` 一样**只是库**、不接动词、不接 schema（§113/§114）；`searchindex.py` 建的是 crawl 索引，**不是 USN 索引**（§32） |
...
| `cli/tests/` | pytest L0/L1 套件（**1457 项**，含 115 项常驻跨工件一致性审计 `test_l0_consistency.py`；`test_l1_agent_read_fields.py` 用真实 root 逐个跑出 agent 面文档，验证 `agents/airoot.json` 的 **177 条 `read` 路径**全部存在）+ `fixtures/golden/`（Rust 版验收语料 **44 个 fixture** + 五个契约目录：`transaction_transitions.json`、`invariant_catalogue.json`、`frozen_capabilities.json`、`scenario_ledger.json`（**108 个场景编号**的完整台账）、`execution_bounds.json`）+ 九个非测试模块：`conftest.py`（注入 `cli/app` 到 `sys.path`、建并清理 `cli/tests/.tmp/`——**删不掉就报错**，§139、会话级"不污染宿主机"断言）、`golden.py`、`fake_issuer.py`（**测试路径的**批准签发方）、`schema_walk.py`（**schema 枚举遍历的唯一定义**，§104）、`fake_broker.py`（进程内 loopback harness，**不是 broker**，§110）、`scenario_ledger.py`、`execution_bounds.py`（两个契约目录的解析器与处置表）、`real_machine_acceptance.py`（真机验收脚本，非 pytest；含宿主快照的隔离审计，§133/§134；§148 起 `--online` 里多一个 `online_install()` phase——真实归档的下载/解压/真跑/gc 真删，已观察到 `real artifact install: PASS`；§149 起它同时登记 `D:\env_apps`（只读）并把能力账本断言成 `{build, ffmpeg, java, node, git, python}`，且该树已进隔离快照）、`dump_tables.py`（开发辅助，不参与断言）。上面每一个 fixture 与辅助模块的说明见草案 §138.3 |
| `cli/app/airoot/policy/discovery-whitelist.json` | **能力白名单**：按 `capability_id` 的证据谓词（不是目录名）；缓存/GUI/服务排除名单（revision `wl-6`；`wl-6` = §141 冻结 `ffmpeg` 并给它条目——名字 + `is_pe` 的**弱证据**，因为这台机器的 gpl-shared 构建没有版本资源；上一版 `wl-5` = §121 / ADR-0048 给 `rust-toolchain` 补上"识别"那一半，两条谓词缺一不可——名单与 PE 产品串） |
| `cli/app/airoot/policy/sources.json` | **可信来源清单**（`src-2`）：允许的 host、每个能力的 artifact 与校验和 URL 模板。**不含 digest**——摘要只来自上游校验和文件。`src-2` = §151 加了**第一个 runtime 来源**（`node`，`nodejs.org` 的 `SHASUMS256.txt` + 那个 `.zip`）——它让操作者有了一条通往 owned runtime 的路：`source resolve node` 走 `portable_archive`，计划是 `kind=runtime`（§150），装出来的就是 `runtime-instance` |
...
| `cli/app/airoot/caps/backends/` | **Install Backend**：`base.py` 九个冻结声明 + 九步协议；`portable_file`（本地无脚本 artifact）、`https_artifact`（HTTPS + 强制 SHA256）、`portable_archive`（**解压** zip；`source resolve` 遇到 `.zip` 选它，§144/§145——入口点是**载荷根之下的相对路径**，ADR-0052 把两个 owned-instance schema 的裸名约束改成相对路径才让它装得进去） |
| `cli/app/airoot/broker/` | **P2 的线路面**（§108/§114/§115）：`protocol.py` 造/解两个 `broker-*` 文档（`build_request` 返回前过 `validate_self`；`parse_response` 只在 `status=ok` 时返回，否则**用响应自己的 reason code 抛错**，把对方的裁决原样带回来）；`transport.py` 是**显式长度前缀的帧**（4 字节长度 + 负载、256 KiB 上界，超了**不读负载**就拒）；`pipe.py` 是一条**真实的** named pipe 的服务端与客户端（答 `probe_root`，把 `commit_plan`/`recover_transaction`/`gc_apply` 拒成 `NOT_IMPLEMENTED`，**拒绝从提权 token 启动**）；`policy.py` 的 `admit_caller(identity, expectation)` 只吃**观测**到的 `ProcessIdentity`（请求里的 `client` 块结构上进不来），`allowed_sids`/`minimum_integrity` 是**显式入参**、没有策略来源；`broker_unavailable()` 是"这一版没有 broker"的诚实回答（`NOT_IMPLEMENTED`(1)，ADR-0025 D1）。**它不是受保护 broker，也不是信任边界**：没有提权进程、没有策略来源、没有任何 CLI 动词走到它，同用户进程能绕过它做的一切——这里只放"不被信任的客户端"该有的东西（ADR-0034）；它到底保护了什么，见 §115.4 |
...
| `cli/app/airoot/caps/toolstate.py` | **只读观察面**：`tool list/status/verify`（整树 digest + 入口点存在性，绝不执行 payload、绝不修复）。`lifecycle_status` 报的是**派生值**（`active` 只在绑定确实指向它时报），记录值在 `recorded_lifecycle_status`——§166 / ADR-0065 |
| `cli/app/airoot/caps/pathexposure.py` | **PATH 不变量检查**：重复 AIROOT 条目 / store 版本目录 / 非授权目录 / launcher 缺失；永不写 PATH。`path repair` 是它唯一的**写一侧**（只补写/重写稳定入口，不删文件，§165 / ADR-0064）|
...
| `cli/app/airoot/policy/capabilities.json` | **冻结能力清单**（`cap-4`）：能力身份/kind/入口/副作用上限/scope。白名单加载期校验"条目必须在此清单内" |
...
# 建 root（第一个动词，不需要提权、不写 PATH；两个身份是显式入参，P1 不替你编）
python -m airoot root init D:\env_test\.airoot --root-instance-id root-env-test --machine-id host-home-d --json

# 真实 CLI（需要先有一个 root：上面的 root init，或 --root/AIROOT_HOME 指向已有的那个）
...
python -m airoot --root <root> plan build --source-json <resolved.json> --json   # 构造真实 artifact 计划；版本来自解析结果（§156），不要另传 --version
...
python cli\tests\real_machine_acceptance.py --online   # 外加 §59 的解析+下载+摘要校验，以及 §148 的
                                                        # 真实归档全链（解压 → 真跑 → gc 真删；临时 root 里签发）
```
...
- **不要把 `where <cap> --version ">=3"` 这类约束直接写进 `cmd /c` 的引号里**：`cmd` 会把 `>` 当重定向，于是当前目录多出一个名为 `22` 的文件（内容是一份 `INVALID_INPUT` 信封），而命令本身安静地失败。§159 的实测就是这么在仓库根留下一个杂散文件的——`git add -A` 之前先看 `git status`。
...
- **§119 收掉了 ADR-0046 漏下的那句话**：裁决改了结论，而五个面还在说旧结论（"这一版没有生产签发方"）——**同一个拒绝语自己讲两个故事**（`ISSUER_PENDING` 已指向 ADR-0046，它下面的 `evidence` 还写着唯一的签发方是测试用的）。**它为什么能活下来**：`test_l1_field_values.py` 的写入者检查只跑一个方向（点了写者却没有构造函数），而 ADR-0046 是**造出**写者的那一次，所以"表格说没有写者"这一半没人查。现在两个方向都有守卫，而且**两边都是推导出来的**（表格里"没有写者"的 schema 必须恰好等于 `docs/schema/README.md` 那份由校验调用点推导的清单）。**这一条同时是一份清单**：措辞改对了不等于下一个同类不会再长出来，能证明的只有被守卫盯住的那两处——其余的靠读者按同样的问法去查。全过程（含第二个同类：`broker-response` 的 `unbuilt` 豁免早已失效）见草案 §119.2/§119.4。
...
允许说：“方案契约与验证计划已具备，P1 协议级 Core 已通过验收；管家域的**非拥有式登记**（数据根 → 只读发现 → reference，含多版本与活跃版本观测）、**依赖分流决策**与 **user 级环境变量持久化**（plan → approval → 写入 → 精确还原）、**steward-first `where`** 与**数据根/reference 诊断**、**删除分级**（只删 AIROOT 自己装的，reference 永不 `uninstall`）、**能力边界**、**Skill 适配层**（`SKILL.md` 是 Skill 根的唯一入口，带漂移守卫）与**可信来源清单**（digest 只来自上游校验和文件）、**`rebuild`**（派生状态可重建、权威不可自重建）、**session 激活的完整语义**（快照栈 + 恢复式 `deactivate` + `SESSION_STATE_STALE`），以及 **`search` 的协议面与 crawl 建的持久索引**（请求/响应按已发布 Schema；实现上限、cursor 绑定索引 generation、root 规则齐备；索引健康时退出码 0 并给出 `freshness=current`，覆盖不足或损坏时如实回落 crawl）已可对真实 `D:\env` 运行（1457 项测试、旧切片无回归、`cli/tests/real_machine_acceptance.py` 全项通过，真机上 `search refresh` 为真实数据根建了 **5 万余条记录**的索引（本轮实测 51 183，§133），且 `data_root_files_touched=0`、`search status --probe-native-index` 报出真实卷读数）。生产实现（受保护 broker/ACL、USN 常驻索引器）与依赖它们的 machine 级持久化待后续。”
...
> **末行哨兵（§138）**：读到本行 = 本文完整到达；没读到 = 你手里是被截断的副本（草案 §138 量过：这里的预算不是这份文件一个人的）。
