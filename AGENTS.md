...
**逐阶段的实现记录**（`§31`–`§133`：每一阶段做了什么、发现什么缺陷、加了哪组守卫、怎么验红、哪些边界没有做）**全部在 `docs/AIROOT-v0.3-管家模型与数据根契约草案.md` 的对应小节里**；本文只保留当前状态、仓库地图与操作面。这是**有意**的：本文是**入口文档**，必须能被读者**完整**载入，而逐阶段细节曾让它长到超过读取预算并被**静默截断**（见 §54）——把细节留在这里，等于让读者拿到半份规则；转写到草案里则**不丢任何信息**（§1 里原来的 23 段在草案里各有一节，实测零独有，见 §54.2）。
...
| `docs/AIROOT-v0.3-管家模型与数据根契约草案.md` | **提案 + 逐阶段实现记录（§1–§30 是契约章节，§31–§133 是每一阶段的实现记录）**：reference-first 管家模型、数据根、能力白名单、`where` 优先级改写、**依赖分流与确认、环境变量持久化、删除语义分级、能力边界声明** |
...
#   整段运行被**两次宿主快照**夹住：HKCU/HKLM 环境块、`.airoot`/`.cargo`/`.rustup` 的逐文件清单、
#   以及 `D:\env` 的汇总（文件数/字节/最新 mtime）。任何一处动了即失败，而且每次运行都用三次合成
#   变异证明这个比较不是空对空（草案 §133）。只有 `--online` 会碰网络。
...
- 远端是 **public**：不要把本机凭据、卷序列号、USN journal ID、真实用户名等指纹写进任何被提交的文件。
  ...
  **它随后被从历史里重写掉了**（草案 §132）：§131 只做到当前树，而这一条要做到"`main` 上没有任何提交
  ...
  26 个被引用的 id 全部重指。**本机仍留着一个含真值的备份 bundle**，等操作者确认后再删；
  GitHub 一侧的不可达对象与已克隆的副本不在本节能承诺的范围内（§132.4 写了这一点）。
