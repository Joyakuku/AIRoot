...
def test_p1_exit_condition_one_state_and_protocol_without_real_software(capsys, prepared, tmp_path: Path) -> None:
    ...
    # The plan is only executable with a token, and a plan document is not an approval token. Minting one
    # is a separate, explicit step (`airoot issue`, ADR-0046/ADR-0049) — this call is `install`.
