...
- 每个事务边界都有故障注入和恢复预期；
- `where`/`doctor`/`search` 的所有机器可读结果都有固定 fixture（**守卫第二十九、三十、三十一组**：第二十九组把 `where` 那张场景表与语料双向对账；第三十组要求每个 fixture 记录的退出码等于**它自己文档**推出的那个，并要求 schema 里 `doctor.status` 的每个取值都出现在某个 fixture 里；第三十一组把同一条覆盖率规则用到 `search.status`，**例外从 `references/field-values.md` 的 † 标记读**——§87/§88/§89 之前，`doctor_healthy` 的名称、状态与退出码三者互不相符、`healthy` 一个 fixture 都没有，而 `timed_out` 明明可产生却没有 fixture）；
- 测试不会污染开发机环境；
