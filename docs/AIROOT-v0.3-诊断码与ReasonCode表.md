...
| D4 | 根内/数据根内无可用登记结论的对象只报告不接管（`UNMANAGED_OBJECT_PRESENT` 仅 `--include-unmanaged`） | `ORPHANED_STORE_INSTANCE`, `UNMANAGED_OBJECT_PRESENT` |
