...
| 5 | privilege required | `ACL_MISMATCH`, `CALLER_NOT_AUTHORIZED`, `PRIVILEGE_REQUIRED` |
...
- `ACL_MISMATCH`（5）在 P1 不会被发射——P1 没有 ACL 与 broker；保留映射以便 P2 直接使用。
  ...
  ——重复过的这一句就是它出错的方式（§108）。
- `CALLER_NOT_AUTHORIZED`（5）是 §114 为受保护 broker 的**第一条判定**新增的码：它说的是"**这个调用方
  没被授权**"，不是"提权再试"——提权不会把 `S-1-5-21-…` 换成另一个 SID。落 5 是因为冻结的 0–9 里
  "操作需要调用方不具备的权限/授权"就是这一层，而没有任何一条既有码是对"**谁在问**"的判断：
  `ACL_MISMATCH` 讲目录的描述符，`OWNERSHIP_REQUIRED` 讲 AIROOT 不拥有的 payload（§114 量过，ADR-0041
  裁决）。**它今天唯一的写者是 `broker/policy.py` 的 `admit_caller`，而没有任何动词能走到那里**，
  所以真机上同样遇不到——"有写者但没路"与"没有写者"是两件事，两者的区别见
  `references/reason-codes.md` 文末。
