...
4. **`rebuild.py` 的同类扫描没有跟着扩展**：它只重写派生投影、并且**只报告陌生对象**（不接管）；`doctor` 现在会报的那件事不在它的职责里。这是**已知的不对称**，记在这里而不是顺手改（改它要动派生重建的语义）。
...
| S68.7 计数与语料 | ✅ | 测试 **781 → 782**；审计检查 **74 不变**；台账计数**不变**；无需重生语料（只改文本与 `notes`，没有改动任何对外 JSON 形状） |
