...
3. **§85 的变异是照着当时的文本抄的字面量**（`主规划（2249 行）`），§86 把规划改长了 9 行，那个变异**静静地不再生效**（`stale == text`，断言直接红在"变异没应用"上）。这是同一类第三次：§84 修了链尾变异的字面量、§85 发现"只从链里推一半"、§86 发现**行数**那一个也是抄的。修法照旧——把两个数都从地图行里推出来。**三次都发生在"照着今天的文本写一个明天会变的字符串"上**，所以下一轮的第一件事是把这条做成检查（见 §87 的打算）。
