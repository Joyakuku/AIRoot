...
| `ed12221` | §54 | 747 |
| `7219b4d` | §60 | 759 |
| `9bfc33d` / `7909192` / `4598e6f` | §66 / §67 / §68 | 776 / 780 / **782** |
| `413c2b9` / `bb560f8` / `e26221d` | §74 / §75 / §76 | 803 / 807 / **809** |
...
|---|---|---|---|---|
| §50 | §51 | 737 | 739 | `16cffdf`（ADR-0021「放宽优先」+ 状态机边集扩到 62 条）加了 **2 条**守卫却**没有新增阶段小节**——它改的是已有小节。§51 的"从"因此从 739 起，这不是笔误。 |
...
2. **守卫看不见 git。** 它只能检查"断口有没有被声明"，**不能**检查声明里那句"因为 `16cffdf`"是不是真的。那句话的验证方式写在 83.2 里（在提交上加 worktree 跑收集），是一次**人工**动作，不是常驻检查。
...
`docs/AIROOT-最小版本-v1.md` 在 **`607cfd9`** 时的 §5：表格 16 行全是 ✅，而表格**下面**那段仍逐字写着：
...
| 变异 | 期望 |
|---|---|
| 把收尾句换回 `607cfd9` 的逐字原文 | 报"表里 16 条全是 ✅，而收尾句没这么说" |
...
把 `607cfd9` 那段逐字原文还原之后，守卫报（pytest 把非 ASCII 转义了，这里解码写回）：
...
引入提交是 `40f4937`（`git merge-base --is-ancestor 40f4937 8d9e98c` 实测为真），也就是它**在这次推送之前
...
| 公共历史 | **未重写**：真值仍在公共仓库的历史提交里（`40f4937` 及其后代）。要清历史得 `filter-repo` + force-push，那是**另一个决定**，也是本项目至今唯一一处"已知仍然公开、且已如实记下"的东西 |
