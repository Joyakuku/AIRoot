...
2. **守卫第二十二组（2 项）**，两项分别守这条修法的**两个危险方向**：
   * **(a) 尺寸**：`AGENTS.md` 不得超过 `ENTRY_DOC_BUDGET_BYTES = 65536`。危险方向是"文件长过了预算却没人发现"
     ...
     不写成一个散落在断言里的魔数。
     （**§138 改了这条**：那里量到的是这份文件**到不了** 65 536——四次载入的落脚点是 65 142–65 243，所以断言变成了`文件 + 余量 ≤ 预算`，并给入口文档加了一个末尾哨兵。这一句保留为当时的裁决形状。）
...
| 行为 | 无产品行为变化 |
## 138. 入口文档的预算是共享的——`≤ 65536` 量的是文件，而契约是"整份到达"

### 138.1 读数：守卫是绿的，读者的尾巴还是被切掉了

§54 立下面这条守卫时，把部署的 65 536 字节当成了**这份文件一个人的**预算：断言是
`len(AGENTS.md 的字节) <= 65536`。它一直是绿的。而这一轮，同一份文件被载入方**截断**了三次，读数就写在
载入记录里：

| 文件字节 | 实际载入 | 被切掉 | 当时文件里刚发生了什么 |
|---|---|---|---|
| **65 489** | 65 242 | 247 | §137 记完，守卫看是对的 |
| **65 489** | 65 142 | 347 | 同一份文件，下一次载入 |
| **65 710** | 65 243 | 467 | 只加了 §1 里那句"本文最后一行是哨兵" |

三次被切掉的一律是**§9 的尾巴**（"维护远端仓库"最后一节）；第三次切掉的正好是半句话。

**差值不相等这件事本身就是结论**：载入的是"工作区指令"这个整体，前置说明与注记都从同一个 65 536 里扣，
所以那份文件**不是一个人的**——占用 247–467 字节、**每次还不一样**，而**从仓库内部测不到它**。
守卫因此有了一个比"数字太小"更糟的形状：**它量的是我们控制得了的量（文件大小），契约却挂在我们控制不了的
量（到达）上**。文件没超界，读者照样丢尾巴——§54 那个"静默截断"的失效模式，被一个**建模错了对象**的界
重新打开。

### 138.2 搬进草案：`caps/` 逐个模块的职责

入口文档 §2 的那一行原本是一篇 27 个模块的散文（**3 151 字节**）。搬走之后它剩下**文件名清单**
（守卫要求每个模块都被点名，见 §85 那一组）与**三条读代码前必须知道的**（`layout.py` 是 payload 标记
扫描的唯一定义、`acl.py` 的写一侧只是库、`searchindex.py` 不是 USN 索引）——**869 字节**。
下面是从那里搬来的原文，逐字：
...
### 138.3 搬进草案：`cli/tests/` 的 fixture 与辅助模块

同一次，入口文档 §2 的 `cli/tests/` 行从 **2 382 字节**收到 **1 253 字节**——留下的是计数、`fixtures/golden/`、
五个契约目录名与九个非测试模块名（守卫要求这些名字都在仓库地图里）。搬来的原文：
...
### 138.4 删掉但**没有**搬：三段阶段叙述与一行模块细节，各自都有更全的家

入口文档里被删的另外四处不是"摘要"，而是**阶段叙述**——它们在草案里本来就有一段比它更全的记载，
所以这次的处理是**指针**，不是转录。逐条实测（都是 `grep` 得到的位置，不是"我记得写过"）：

| 入口文档里删掉的 | 草案里更全的记载 |
|---|---|
| named-pipe：`PIPE_REJECT_REMOTE_CLIENTS`、DACL、"对端 pid 取自 OS"、"拒绝从提权 token 启动"、帧的 256 KiB 上界 | §115.1（那个标志位不在 MSDN 说的参数里）、§115.4（这条线到底保护了什么）、§115.7（如实记录的边界） |
| 真实安装：12 721 664 字节、SHA256 逐字节相同、19.1 秒、`rustc 1.98.1 / 1.29.1`、`.cargo` 12.1 MB + `.rustup` 576.0 MB | §117.4（四步与它们的读数）、§118.3（实测） |
| §119 那个同类**为什么能活下来**（写入者检查只跑一个方向）与第二个同类（`unbuilt` 豁免失效） | §119.1–§119.4 |
| `broker/` 一行的细节（`serve_pipe`/`call_broker`、`admit_caller` 的形状、`allowed_sids` 没有策略来源） | §108（客户端线路面）、§114（服务端第一条判定）、§115（pipe 与它保护了什么） |

**这一节的做法与 §54 一致，但边界不同**：§54 搬的是"独有"的段落（实测零独有之后才搬）；这里删掉的
是"**另有其家**"的段落，所以只留指针。**指不到的就不删**——上面四条是这一轮逐条 `grep` 过的。

### 138.5 修法：一条是余量，一条是哨兵

1. **余量**：`ENTRY_DOC_BUDGET_BYTES` 保持 65 536（那是部署的数），断言改成
   `文件字节 + 余量 <= 预算`，余量取 **2 048**——实测占用 247–467 且每次不同，余量取到实测上界的四倍。
   理由是**这个数从仓库内部测不到，所以只能宁可早响**；余量的出处（三次读数）写进守卫的注释里，
   不抹进断言。同时保留 §54 就有的下限（文件不能靠"几乎为空"来通过）。
2. **哨兵**：文件最后一行是一句自述（`> **末行哨兵（§138）**…`）。**它让"截断"这件事对读者可见**：
   读到它=整份到达；没读到=手里是被切过的副本。光有末尾哨兵还不够——**一个从没读过完整文档的人，
   不会知道末尾本该有什么**，所以 §1 里还有一句声明（"本文最后一行是哨兵"），它必须**在前面**，
   因为截断切的是尾巴。守卫两条都守：末行恰是它、全文只出现一次。

### 138.6 实测：改完之后的那次载入

| 时点 | 文件字节 | 载入结果 |
|---|---|---|
| 只加了 §1 那句声明 | 65 710 | 仍被截断（65 243），切掉的是 §9 尾巴的后半句 |
| 瘦到 60 124 之后 | **60 124** | **没有截断记录，且末行哨兵在**——整份到达 |

**这是这一节里唯一不需要相信叙述的那句话**：哨兵在不在，是载入记录自己说的。

### 138.7 成本

| 项目 | 结果 |
|---|---|
| 入口文档 | 65 489 → **60 124** 字节（净 **-5 365**）：距部署预算 5 412 字节、距自设界（65 536-2 048=**63 488**）3 364 字节 |
| 砍在哪里 | §1「尚未实现」3 911→2 372、`caps/` 行 3 151→869、`cli/tests/` 行 2 382→1 253、`broker/` 行 1 787→1 258、§119 条 1 334→1 037；另加 §1 的哨兵声明 **+221**、末行哨兵 **+186** |
| 搬走的正文 | 两处，逐字在本节 §138.2/§138.3；四处只留指针，出处见 §138.4 |
| 测试 | **1368 → 1368**（同一个守卫加严，**不新增用例**，所以 `test_l0_consistency.py` 的 111 也不动） |
| 守卫 | 预算一组：+余量（带三次读数的出处）、+非空性两侧（界内通过 / 界外报出 / 地板）；末尾一组：哨兵位置与唯一性；两组都用合成变异验红 |
| 行为 | 无产品行为变化 |

### 138.8 补一条读数：§54 自己就记着一次，而它换掉了这一节的模型

写完 §138.1 之后顺回 §54 核了一遍，那条守卫当时记下的载入报告是：

```text
truncated AGENTS.md from 65840 to 65243 bytes
```

**落脚点 65 243，与本节第三次读数一模一样**（65 710 → 65 243）。于是这一节的模型要改一句：

| 口径 | 读数 |
|---|---|
| 被切掉的字节数 | 247 / 347 / 467（本节）/ **597**（§54） |
| **落脚点**（载入真正留下的字节数） | **65 142 / 65 242 / 65 243 / 65 243** |

要量的是**落脚点**，不是"被切掉多少"：后者等于"文件大小 − 落脚点"，所以它随文件大小变，**看不出规律**；
前者四次里有三次落在 65 242–65 243、另一次 65 142，跨度只有 **101 字节**——也就是说这份文件能占的实际上
是 **≈65.2 KB，而不是 65 536**，中间那道缝是 **293–394 字节**。

所以 §138.5 里那句"余量取到实测上界的四倍"用错了口径：**2 048 不是 467 的四倍，而是那道缝（293–394）的
五到七倍**——而它要防的正是那道缝**继续变宽**（多一份指令、多一段前置说明）。守卫的注释按这个口径写了。

**这一节同时说明 §138.1 的那张表是不完整的**（少了 §54 这次），而它**不重写**：那张表是当时量到的三次，
这里是第四次。**同一份文件里两句话描述同一件事不一致**是这个项目一直在抓的形状，所以 §54 那句旧规则
也在原地标了被改（`文件 + 余量 ≤ 预算`），而不是让它留着一个已经不成立的读法。

### 138.9 验红：拿真实文件做的两次变异（不是只对着 helper）

守卫自己带着非空性，但那只证明"这段判据会红"，不证明"它对**这份文件**会红"。于是又用手改真实文件的方式
量了一次（每次量完 `git checkout` 还原，前后 sha256 相同：`31956a9e3e6700ac`）：

| 变异 | 意图 | 结果 |
|---|---|---|
| 尾部加 3 000 字节 → **63 124 字节**（**仍然小于 65 536**） | 走一遍 §138 那道缝 | **红**——旧规则在这个尺寸上是**绿**的，这就是这一节的整个论点 |
| 删掉末行哨兵（尺寸不动，59 932 字节） | 尾巴没了，但尺寸毫无问题 | **红** |
| 还原后 | 对照 | 绿 |

## 139. 每跑一次测试就漏一个 root——因为清理失败是**静默**的

### 139.1 现象：三个读数，全是"测试通过、残留留下"

| 怎么跑的 | 结果 |
|---|---|
| `pytest cli/tests`（整轮） | 全绿，`cli/tests/.tmp` 里留下**恰好一个** `airoot-test-*`（两次实测：`8eeyemrm`、`9mkt7mfi`） |
| 单跑 `test_l2_fake_broker.py` | 36 项全过，留下 **6 个** |
| 那棵树里有什么 | **只有** `state/registry.db`——不是"没建完"，是 `rmtree` 一路删到底、**停在被占用的那个文件上**，其余全删掉了 |

**"只有 registry.db"这一条就是定位线索**：留下的不是完整的 root，是一棵被测树被删到一半的**残骸**。
还有一条：它是**自愈**的——上一轮留下的那个，会在下一轮 session 收尾时被删掉（实测 `8eeyemrm` 消失）。
所以这不是磁盘在涨，是**一次静默的清理失败盖住了一个没有关闭的句柄**。

定位过程本身也值得记一句：**逐模块各起一个 pytest 进程**跑一遍，哪个模块后面有残留就是它——
36 个模块里只有 `test_l2_fake_broker.py` 复现。

### 139.2 根因：开的人不关

`cli/tests/fake_broker.py` 的 `serve()` 每次都 `Registry.open(...)` 放进 `_OperationContext`，然后
**从不关闭**——连接的寿命交给垃圾回收。多数用例里 refcounting 恰好在 teardown 前把它放掉，所以只有
一部分漏（单跑 6/36）；一旦某条路径把上下文留在别处（注入器、恢复引擎留下的证据），它就活到进程退出，
而被占用的 `registry.db` 在那之前一直拒绝删除。

### 139.3 这条教训**早就有人学过**，只是没进共享夹具

`test_l1_acl.py` 里有一条 `_delete_directory`：`rmtree(ignore_errors=True)` 之后**断言它真没了**，
注释写着"deny ACE 让删除被拒绝，静默地留下目录"。也就是说，**同一个项目里，一个模块自己学会了这件事，
而所有模块共用的那两处 `conftest` 清理还在吞掉失败**。§139 的动作就是把这个本地教训变成**共享定义**：
`conftest.remove_test_tree` 现在是"删掉一棵测试树"的唯一定义，两处会话/用例夹具与 `test_l1_acl.py`
的 `_delete_directory` 都调它。

### 139.4 改了什么

1. **`conftest.remove_test_tree(path, *, attempts=10, pause=0.15)`**：重试瞬时锁，然后**报出还剩下什么**
   （断言，附带存活路径与前一个 `OSError`）。重试不是宽容——它区分"别的进程暂时拿着"与"本进程一直不放手"，
   只有后者是失败。
2. **`fake_broker.serve`**：`try/finally` 关掉自己开的 registry。原来那段正文变成 `_dispatch(...)`，
   由调用者持有 registry 的生命周期。
3. **一个新守卫**（`test_l0_consistency.py`，常驻审计 **111 → 112**）：真锁住的文件**必须报**
   （`open()` 在 Windows 上不共享删除，这正是旧 teardown 藏起来的那种情形），放开之后**同一个调用静默删掉**
   ——两个方向都测。

### 139.5 验红：把漏放回去，看它会不会喊

| 变体 | 结果 |
|---|---|
| 把 `finally` 里那句 `close()` 换成 `pass`（= §139 之前的行为） | 36 项**仍然全过**，但 teardown 报 **22 个 error**、`.tmp` 里 **17 棵**残留；错误里点名每一棵树与 `PermissionError(13, '另一个程序正在使用此文件…')`——**以前这里是一个字都没有的** |
| 恢复修复后 | 36 项全过、exit 0、**0 棵**残留 |
| 修好之后再跑整轮 | 0 棵残留（会话收尾也不再需要"清上一轮的"） |

**"测试全过 + 22 个 teardown 错误"这一对读数是这一节的核心**：它说明清理失败与用例判决**本来就没有关系**
——所以旧行为才可能连着好几轮没人发现。
...
| 项目 | 结果 |
|---|---|
| 测试 | **1368 → 1369**（+1：清理守卫）；`test_l0_consistency.py` 常驻审计 **111 → 112** |
| 计数同步 | `AGENTS.md` 三处（§1 已实现段、§2 `cli/tests/` 行、§8 允许说法）、规范审查报告的当前状态节两处 |
| 行为 | 无产品行为变化；**测试夹具**的清理从"吞掉"变成"报出" |
| 真机脚本 | 无改动 |
| 新增文件 | 无（`remove_test_tree` 进 `conftest.py`） |

### 139.7 让它喊的收益：第一轮全量跑又抓出两处，其中一处在**核心**里

把清理从"吞掉"改成"报出"之后，第一轮 `pytest cli/tests` 就报了 **3 个 teardown error**——全是
"故意把 `registry.db` 写坏"的那三个用例（`test_l1_registry` / `test_l1_doctor` / `test_e2e_p1`）。
它们各自留下一棵删不掉的树，于是同一个问法往下问了一层，又抓到两处（加上 §139.2 那一处，一共三处）：

| 位置 | 缺陷 |
|---|---|
| `Registry._connect`（**核心**，不是测试） | 那四个 `PRAGMA` 里任何一个拒绝，**刚建的连接就地泄漏**——`file is not a database` 正好落在这里，所以"把库写坏"这条路径每次都在锁住那个文件 |
| `Registry.open`（**核心**） | 只捕获 `sqlite3.DatabaseError`；`_require_tables()` / `_apply_migrations()` 自己拒绝时连接没有关闭，而 `raise … from exc` 又把那一帧留在 traceback 里，于是锁一直活到循环回收器动手 |
| `registry` 夹具（`conftest.py`） | 与 `serve()` **同一个形状**：夹具开的连接，从不关闭 |

三处都改成"谁开谁关"（**失败路径也关**）。**前两处不是测试卫生问题**：`Registry.open` 失败时漏掉一个
句柄，在任何长跑进程里都是句柄泄漏——它以前唯一的可见症状是"某个目录删不掉"，而那个症状正好被
`ignore_errors=True` 吞掉了。修好之后整轮跑完：**1346 passed + 23 skipped，`.tmp` 0 个条目**。
...
## 142. 那个 400 文件的上限会把浅入口挡在深树后面（`discover` 改成**先浅后深**）

### 142.1 读数：同一棵树、同一个谓词、两种答案

`git` 的谓词**本来就是对的**——`cmd/git.exe`、`bin/git.exe`、`mingw64/bin/git.exe` 四个候选全是
`product='Git'`、`company='The Git Development Community'`、`file_version='2.49.0.1'`，
`match_entry` 逐个返回 `True`。可是这台机器上 `D:\env_apps\Git` 在 `discover` 里是 **unmanaged**。

原因在扫描预算上，不在谓词上：

| 事实 | 读数 |
|---|---|
| 每个对象一次最多看多少可执行文件 | `max_files_per_object = 400` |
| 那次扫描 | `found=400`、**`capped=True`**，前 12 个命中里没有 `git.exe`，最深已经到 `depth=3` 的 `usr\…` |
| 走法 | 深度优先（显式栈 + `pop()`），而 `Git\` 下 `usr\` 比 `bin\`/`cmd\` 大得多 |

**结论**：`truncated` 本来是诚实的（它报了 True），但**截断策略决定了哪些对象能被认出来**——
深度优先会把预算花在第一个大子树上，浅处的入口点因此永远看不到。这与"最浅命中胜出"的既有选择规则
是**相反**的方向。

### 142.2 修法：广度优先

`_iter_executables` 从显式栈改成队列（`deque.popleft()`）：先看完第 0 层所有文件，再进第 1 层，
依此类推。预算还是那个预算、`truncated` 还是照报，只是**花预算的顺序**与匹配器的顺序一致了。

### 142.3 真机读数（改后）

| 数据根 | 结果 |
|---|---|
| `D:\env` | `external_reference=4`（`build`/`ffmpeg`/`java`/`node`）**与改前一致**，`truncated=False` |
| `D:\env_apps` | **`Git` → `cap=git`，version `2.49.0.1`**；`miniconda3` → `cap=python` `3.12.9150.1013`；unmanaged **1 → 0**；仍 `truncated=True`（那棵树本来就大，报出来是对的） |

### 142.4 守卫与验红

新用例（`test_l1_discovery.py`）：一份**预算只有 3 个文件**的 git 白名单 + 一棵 `Git/` 树
（`bin/git.exe` 在深度 1，`deep/` 里放 5 个可执行文件）——顺序决定答案。

| 变异 | 结果 |
|---|---|
| 把 `popleft()` 换回 `pop()`（= 深度优先） | **红**（同一个用例：入口点被深子树挡掉） |
| 还原 | 绿，前后 sha256 相同 |
...
| 项目 | 结果 |
|---|---|
| 测试 | **1375 → 1376**（+1）；常驻审计 **112 不动** |
| 行为 | 每个对象的扫描顺序变成"先浅后深"；上限、截断语义、候选选择规则都不变 |
| 协议面 | 无新 schema、无新码、无新动词；`discover` 的输出形状不变 |
| 真机 | 同上（§142.3）——**这一条正是"账本描述真机"的第二次实测** |

### 142.6 补：这条记录自己落了两次——于是加了一条守卫

上面的 §142 在本轮里被**追加了两次**（一个非幂等的追加脚本被重跑），而它进了提交。这不是文档措辞问题，
是**记录本身**多了一份：范围句指向的编号是对的、最新小节是对的、计数还是对的，只有"§142 出现了两次"
这件事没人看。

所以补一条守卫（`test_l0_consistency.py`）：**每个 `## N.` 阶段编号恰好出现一次**，
并把"取编号"抽成 `_stage_numbers` 一个定义（原来的 `_newest_stage_section` 也改用它）。
非空性方向就是这一次的真实形状：把最新小节再抄一份 → 报出重复。

| 项目 | 结果 |
|---|---|
| 测试 | **1375 → 1377**（**整段**：§142.5 的 +1 加本节守卫的 +1）。写成两行会被读成"1376 起跳"——计数链只认每一节里 `to` **最大**的那一对 |
| 常驻审计 | **112 → 113** |
...
## 146. 同一个名字在一个模块里定义了四次：1382 项测试没有一处看得见（守卫第三十八、三十九组）

这一阶段原本要做的是 **P5 Runtime**。测量的第一遍读 `cli/app/airoot/caps/runtime.py`，就看见
`active_instance_for_capability` 在**同一个文件里出现四次**。它跟 P5 没有关系，但它是**读数**，
所以先按项目规矩把这一处做成一个阶段：P5 的测量从它开始，而它比 P5 更该先修。

### 146.1 先量：三份文件，九个冗余定义

扫过 `cli/` 下全部 **134** 个模块（`app` 67、`tests` 66、`fake_vertical_slice` 1）的顶层
`def`/`class`，只有三份文件有重复，而且**都是四次**：

| 文件 | 名字 | 副本 | 逐字节相同 | 调用点 |
|---|---|---|---|---|
| `cli/app/airoot/caps/runtime.py` | `active_instance_for_capability` | 4 | 是（`sha256` 前 12 位 `b36d8d9d6451`） | 1（`cli.py` 的 `cmd_run` 路径） |
| `cli/app/airoot/caps/where.py` | `_launcher_for` | 4 | 是（`a56d4775190e`） | 1（`where` 的结果文档） |
| `cli/app/airoot/cli.py` | `_expected_launchers` | 4 | 是（`ed87f23f1ae3`） | 1（`cmd_path_verify`） |

`git log -S "def <名字>("` 把三处都指到**同一个提交**：`288e178`（"Give the stable entry a shape and a
writer"，也就是 ADR-0050 那一阶段）。所以这不是逐阶段沉积，是**一次粘贴事故**，而且**已经在远端上**
（`288e178` 是 `23da4c9` 的祖先）。三个名字各自只有一个调用点，于是每份文件里**三个副本是不可达代码**。

同一遍扫描的另外四种形状——类里的重复方法、函数体内的重复嵌套定义、重复的顶层赋值、`__all__` 里的
重复条目——在 134 个模块里**各为 0**。这条读数是后面"守卫该盯哪几种形状"的依据，不是猜的。

### 146.2 为什么 1382 项测试一个都没红

因为**行为面完全正确**。Python 对重复的顶层定义取**后一个**，四个副本逐字节相同，所以取哪个都一样；
调用点只有一个，走的永远是活的那一份。测试问的是"它做对了吗"，而它做对了。

代价不在字节上，在两件事：

1. **一处改在前三个副本上的修复什么都不改，却看起来像修复**——下一次有人顺着 grep 找到第一份定义、
   改掉、跑绿、提交，改的是一份死代码；
2. 读者数调用点会得到"一个调用者、四个定义"，然后必须自己判断哪一份是活的。**代码要读者靠猜才能读懂，
   就是缺陷**，哪怕它跑得对。

这正是这一组守卫存在的理由：**行为测试按构造看不见它**，只有结构性的问题才能看见。

### 146.3 顺手量出来的第二个缺陷：一句会变成 `SyntaxError` 的 docstring

新增的守卫要 `ast.parse` 每一个模块，副作用是**把 import 期的告警翻了出来**：
`cli/app/airoot/broker/pipe.py` 的模块 docstring 里写着 `` `\p` ``（`\pipe\` 那一段），而它不是原始字符串。

| 事实 | 读数 |
|---|---|
| 编译该模块发出的告警 | `DeprecationWarning: invalid escape sequence '\p'`（Python 3.12 起是 `SyntaxWarning`） |
| 全 `cli/` 树里会这样报警的模块 | **1**（就是它） |
| 同一文件第 941 行的同一个写法 | 在**注释**里，注释不参与编译，不报警 |

它一直没被发现，因为 import 期的告警**不是失败**：测试每次都 import 这个模块，每次都把这句打进
warnings summary，而 summary 读起来像噪音。再往后一步它就是 `SyntaxError`，那时这个模块**根本
import 不进来**，整个套件会一起红——**用一次全红换一句早该修的告警**。

修法是把源码里的反斜杠写成两个（`` `\\pipe\\` ``）：**渲染出来的 docstring 文本一字不变**，
因为这条要管的是 Python 怎么读，不是读者看见什么。

### 146.4 改了什么

1. 删掉 **9 处**冗余定义（三份文件各 3 处），按 `ast` 的行号区间删，保留**第一份**：
...
3. **守卫第三十八组**（`test_no_module_defines_the_same_name_twice`）与**第三十九组**
   （`test_every_module_compiles_without_a_warning`），都住在 `test_l0_consistency.py` 里已有的
   `_walked_text_files()` 之上——**"树"只有一个定义**，新守卫不去另建一次遍历。

### 146.5 守卫与验红

**第三十八组**只读**直接的 body 子节点**：模块级的顶层 `def`/`class`/赋值，以及类体里的方法。
`if`/`try` 底下的定义**不报**——那两分支在 Python 自己的语义里互斥，是合法写法，报它等于把结构检查
变成拼写检查。函数体内的嵌套定义也**不在范围内**，理由相同（它的词法作用域包含函数里 `if`/`try`
底下的定义，正确答案要去复现那套作用域规则），**这一形状实测 0 例，写在守卫的注释里而不是默默假定**。

先加守卫、后修代码，红色原文（未修之前）：

```text
AssertionError: a module may define a top-level name once: the later definition silently wins, so a
change made to an earlier copy does nothing while looking like a change:
cli/app/airoot/caps/runtime.py: defined active_instance_for_capability x4 at module level; ...
cli/app/airoot/caps/where.py: defined _launcher_for x4 at module level; ...
cli/app/airoot/cli.py: defined _expected_launchers x4 at module level; ...
1 failed, 113 deselected
```

它一次点名三份文件、每个 `x4`，并说清"后一个赢"——**一处模糊都没有，修法是机械的**。

两组的非空性都在**合成输入**上证明（干净树是常态，一个从不报的扫描与干净树无法区分）：

| 守卫 | 种下去 | 期望 |
|---|---|---|
| 第三十八组 | 重复顶层函数／重复类／重复方法／重复赋值／`__all__` 重复条目／语法错误 | **各自被报** |
| 第三十八组 | `if`/`else` 下的同名模块级定义、类体里 `if`/`else` 下的同名方法、函数体内同名嵌套定义 | **必须不报**（豁免本身也被断言，不是注释里的承诺） |
| 第三十九组 | 非原始字符串里的 `\p`、语法错误 | **各自被报** |
| 第三十九组 | 原始字符串里的 `\p`（`r"\p"`） | **必须不报**——那是修法，不是缺陷 |

两组都先断言"走到的模块数 > 100"，否则这个守卫是在对一个空集合发问。

### 146.6 一处**没有**修的弱点（写在这里，免得被当成已解决）

同步计数时读到 `test_the_skill_states_the_same_test_count_as_the_repo`。它的名字说的是
"SKILL.md 与仓库陈述同一个测试总数"，而它**今天是空对的**：断言先算
`re.findall(r"(\d{3})\s*项测试", ...)`，`SKILL.md` 里没有任何 `NNN 项测试`，于是 `not skill_counts`
直接短路成真——**它从来没有比较过任何两个数**。两个毛病叠在一起：

1. `\d{3}` 读不了四位数：`1384 项测试` 会被读成 `384`（正则从第二位开始匹配）。这一个**今天不致命**，
   因为两边的错误方式相同就会相等；
2. 真正的空对是**一边沉默**：让这条断言不空，要么让 `SKILL.md` 陈述计数，要么把断言改成"两边都必须
   陈述且相等"。前者动的是**面向 agent 的入口文档**（它和 `agents/airoot.json` 有漂移守卫），
   后者会让今天绿的树立刻红。

两件都不是这一阶段该顺手做的事，所以这一阶段**只改正则的一半也没用**（`\d{3,4}` 不会让 SKILL.md 开口），
于是**一处都不改**，把读数留在这里。要做它的时候，它自己是一阶段。

### 146.7 成本

| 项目 | 结果 |
|---|---|
| 测试 | **1382 → 1384**（+2：守卫第三十八、三十九组各一条）；常驻审计 **113 → 115** |
| 代码 | 少 9 处顶层定义（−11682 字节），`pipe.py` +2 字节，两处都不改任何行为 |
