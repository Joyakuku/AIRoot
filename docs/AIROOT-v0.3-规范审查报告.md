...
**当前规模**：`cli/schema/` **19** 个 JSON Schema；`pytest cli/tests` **771 项**（含 **73** 项常驻跨工件
...
| L0/L1 测试与语言无关 golden 语料 | `cli\tests\`（**771 项**）、`cli\tests\fixtures\golden\`（**27 个 fixture**）、`cli\tests\scenario_ledger.py`（**108 个场景编号**的解析器与处置表，含每条 `undesigned` 的证人）、`cli\tests\execution_bounds.py`（**决定拒绝的每个数值**的解析器与来源声明）、`references\confirmation.md`（三选一与"记忆只读"已绑到代码） |
...
- `.ai/tooling.json` 的写入。
- **108 个场景编号里有 56 个没有任何测试点名**（台账 `scenario_ledger.json` 逐条给出结构性理由：
  4 个需要 P2 受保护状态、1 个需要 P4 真实 artifact、10 个依赖尚未设计的能力、40 个由某个测试覆盖
  ...
  **证据指针**而不是覆盖判定；`S-010`/`S-015` 是同一个场景登记了两次，
  ...
  52 个被测试点名。**§61 把 17 条从未复核过的"缺某能力"判断逐条重新推导了一遍**，其中七条改判
  （P-002 / P-016 / T-013 不该记 P2/P4，另外 §60 的 `reconcile` 同属这一类），三条改成
  `undesigned`（P-012 / P-015 / P-017：它们各有一半已交付、另一半**根本没有实现对象**——
  `revoked_at` 无写入方、`events` 表没有 `approval_mode` 列、没有签发方身份证据），其余十条**维持原判**
  并补上"哪一半已交付"。复核还抓出一个真缺陷：同一 token 的第二次 `commit` 会把**已落盘的**
