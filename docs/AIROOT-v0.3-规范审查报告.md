...
**当前规模**：`cli/schema/` **20** 个 JSON Schema；`pytest cli/tests` **1369 项**（含 **112** 项常驻跨工件
...
| L0/L1 测试与语言无关 golden 语料 | `cli\tests\`（**1369 项**）、`cli\tests\fixtures\golden\`（**43 个 fixture**）、`cli\tests\scenario_ledger.py`（**108 个场景编号**的解析器与处置表，含每条 `undesigned` 的证人）、`cli\tests\execution_bounds.py`（**决定拒绝的每个数值**的解析器与来源声明）、`references\confirmation.md`（三选一与"记忆只读"已绑到代码） |
