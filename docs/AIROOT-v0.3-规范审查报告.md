...
**当前规模**：`cli/schema/` **19** 个 JSON Schema；`pytest cli/tests` **765 项**（含 **73** 项常驻跨工件
...
| L0/L1 测试与语言无关 golden 语料 | `cli\tests\`（**765 项**）、`cli\tests\fixtures\golden\`（**27 个 fixture**）、`cli\tests\scenario_ledger.py`（**108 个场景编号**的解析器与处置表，含每条 `undesigned` 的证人）、`cli\tests\execution_bounds.py`（**决定拒绝的每个数值**的解析器与来源声明）、`references\confirmation.md`（三选一与"记忆只读"已绑到代码） |
...
- `.ai/tooling.json` 的写入。
- **108 个场景编号里有 57 个没有任何测试点名**（台账 `scenario_ledger.json` 逐条给出结构性理由：
  4 个需要 P2 受保护状态、1 个需要 P4 真实 artifact、11 个依赖尚未设计的能力、40 个由某个测试覆盖
  ...
  **证据指针**而不是覆盖判定；`S-010`/`S-015` 是同一个场景登记了两次，
  ...
  51 个被测试点名。**§61 把 17 条从未复核过的"缺某能力"判断逐条重新推导了一遍**，其中七条改判
