...
| `$defs.health` | `healthy` / `degraded`† / `broken` / `stale`† / `drifted`† | 对象健康度。这一版只写 `healthy` 与 `broken`（坏掉但**保留为证据**）。注意：ACL 漂移与"索引过旧"**不写进这个字段**——它们分别是 `DATA_ROOT_ACL_DRIFT` 诊断与 `freshness.state=stale`，所以 `drifted`/`stale` 在这里没有写者。**它是登记时记录的值，不是重测值**（§159 F7 量过）：`tool list` / `tool status` 直接取注册表那一列，所以一个 payload 已被 `gc` 收走的实例仍可能读 `health=healthy`——同一份 `tool status` 里的 `payload_present=false` 与 `findings=[PAYLOAD_COLLECTED]` 才是那一刻的事实。**这个 build 里重测的是 `where`**（它会给 `NOT_FOUND`）与 `caps/health.py` 的 `observe_payload`（§147） | `tx/artifact.py`, `tx/simulate.py`, `cli.py`, `registry/db.py`, `registry/entities.py` |
...
| `$defs.lifecycle` | `installed` / `active` / `retired` / `broken`（这四个是本版会写的）；`discovered`† / `external_reference`† / `unmanaged`† / `planned`† / `staged`† / `verified`† / `garbage_collectable`† | 生命周期：`installed` 已提交进 store / `active` 有 active binding / `retired` 已退役（**payload 还在**）/ `broken` 坏掉但留作证据。其余七个这一版不写：登记进 store 就直接是 `installed`/`active`，没有中间态；"可回收"是 `tool gc --plan` **算出来的**判据，不是行上的状态。**读出来的值由代码派生、不是那一列的原样**（§166 F12 §1.1）：`active` 只在当前 active binding 确实指向这个实例时报，而注册表里的记录值是 `active`、绑定却不在它身上时（被新版本取代的那一行）读 `installed`——因为 `active` 的定义就是"有 active binding"，原样报它会让同一份文档里的 `active_binding` 与它互相矛盾。原值保留在 `recorded_lifecycle_status`（`tool list`/`tool status`）| `tx/artifact.py`, `tx/simulate.py`, `caps/lifecycle.py` |
| `$defs.binding.exposure` | `stable_launcher` / `session_env`† / `project_binding`† / `none`† | 绑定是怎么暴露的：`stable_launcher` 是这一版**唯一**写出的（机器级绑定指向 `<root>\cli\exposure\bin\<capability>.cmd`，一个**静态**入口：内容里没有版本，换版本逐字节不重写 —— §123 / ADR-0050）。`session_env`/`project_binding`/`none` 这一版都没有写者——会话激活**不建 binding**，它写的是会话快照栈；而不暴露的实例在这版里根本不建绑定行 | `tx/artifact.py`, `tx/simulate.py` |
